...
mobile: MethodMobile, 
browser: opcion == '1' ? ['FerShop', 'Edge', '20.0.04'] : methodCodeQR ? ['FerShop', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "Fer Shop"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './sessions';
if (!existsSync(subBotDirectory)) {
console.log('𝑰𝒕𝒂 𝑩𝒐𝒕🧊 no tiene Sub-Bots vinculados.');
return;
}
...
console.log(chalk.bold.greenBright(`🌠 𝑰𝒕𝒂 𝑩𝒐𝒕🧊 conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar 𝑰𝒕𝒂 𝑩𝒐𝒕🧊: `, error))
}
})();
...
const directories = ['./sessions/', './KaoriJadiBot/']
